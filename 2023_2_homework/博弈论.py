...
# 初始化各参数
n = 800  # 换电需求数
min_price = 150  # 换电价格范围
max_price = 250
A = np.random.normal(30, 5, 25)  # 初始期望，平均值为30，方差为5的高斯分布
E = np.floor(A)  # 朝下取整
# 下面是根据需求数调整E的大小
E = np.floor(A)
a = sum(E) - n
...
H = np.mat([[2, 2], [2, 6], [2, 10], [2, 14], [2, 18],  # 初始化换电站位置
...
J = []  # 价格变化的差值
ER = [err]  # E-e的变化差值
pbar = tqdm(total=100, desc="Eval_eps", unit="it")  # 初始化进度条
